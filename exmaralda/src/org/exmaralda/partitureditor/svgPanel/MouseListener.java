...
public class MouseListener implements EventListener {
	...
	public MouseListener(SVGDocument svgDoc, JSVGCanvas svgCanvas,
			List<SVGLocatable> pathElements) {
		this.svgDoc = svgDoc;
	...
	public void handleEvent(Event evt) {
		if (evt instanceof DOMMouseEvent) {
			...
			if (target instanceof SVGImageElement) {
				...
				BridgeContext ctx = canvas.getUpdateManager()
						.getBridgeContext();
				...
				for (Object o : ((CanvasGraphicsNode) it.get(0)).getChildren()) {
					...
					AffineTransform inAt = ((GraphicsNode) o)
							.getInverseTransform();
					Point2D p2d = inAt.transform(new Point2D.Float(x, y), null);
					x = Math.round(((float) p2d.getX()));
					y = Math.round((float) p2d.getY());

					if (rect2.getX() <= x
							&& (rect2.getX() + rect2.getWidth() >= x)) {
						if (y >= rect2.getY()
				...
				if (!containingNodes.isEmpty()) {
					...
					for (GraphicsNode node : containingNodes.subList(1,
							containingNodes.size())) {
						if (smallestNode.getSensitiveBounds().contains(
								node.getSensitiveBounds())) {
							smallestNode = node;
						}
					}
				}
